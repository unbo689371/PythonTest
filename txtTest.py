...
class editor():
  def __init__(self,rt):
    ...
    self.t.title("記事本Test%d" % (len(t1)+1))
    
    

    self.frm_file = tk.Frame(rt)
    self.frm_file.grid(row =0,column =0,padx =0,sticky = W)
    self.btn_open = Button(self.frm_file,text = "打開",relief = GROOVE,command = self.openfile)
    ...
    self.btn_saveas = Button(self.frm_file,text = "另存為",relief = GROOVE,command = self.saveasfile)
    ...
    self.editm.add_separator()
    self.editm.add_command(label = "貼上",command = self.paste,accelerator = " "*10 + "Ctr + V")
    self.editm.bind_all("<Control-v>",self.paste)
    self.editm.add_separator()
    self.editm.add_command(label = "剪下",command = self.cut,accelerator = " "*10 + "Ctr + X")
  ...
  def font_it(self,event = None):
    ...
    self.label_size = Label(self.t_font,text = "字體大小")
    self.label_shape = Label(self.t_font,text = "字體形狀")
    self.label_font = Label(self.t_font,text = "字體類型")
    self.label_weight = Label(self.t_font,text = "字體粗細")
    ...
    self.labFra_display = LabelFrame(self.t_font,text = "字體樣式演示區域")
    self.labFra_display.grid(row =2,column =0,pady =4)
    self.lab_display = Label(self.labFra_display,text = "我在這裏")
    self.lab_display.pack()

    self.btn_ok = Button(self.t_font,text = "確定",width = 10,height =2,command = self.change)
